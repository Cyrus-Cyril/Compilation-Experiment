...
## ISSUE-004 2026-07-07 平台评测功能回退与性能超时

- 时间: 2026-07-07
- 位置: `src/semantic/semantic_analyzer.cpp`, `src/backend/code_generator.cpp`, `src/optimize/optimizer.cpp`
- 关联评测: `f09_func_name`, `p07_loop`, `p08_basic_combined`, `p09_advanced_graph`, `p10_advanced_matrix`

### 现象

- 功能测试总分为 96.67/100，其中 `f09_func_name` 显示“编译器异常”。
- 性能测试总分为 5.08/100，`p07-p10` 运行超时，其他性能点虽通过但性能比例偏低。

### 初步根因

- 语义分析按源码顺序声明函数，后定义函数调用会被误判为未定义。
- 非 void 返回路径只检查函数体最后一条语句，对 `if (1) return ...;`、前序 `return` 后有不可达语句等情况过于保守。
- 后端寄存器缓存后，`RET` 仍直接从虚拟寄存器栈槽读取，可能漏读缓存寄存器中的最新值。
- 优化器仍以单趟局部常量传播为主，缺少 CFG、复制传播、公共子表达式消除和循环/尾递归优化。

### 本次处理计划

- 先修复 `f09_func_name` 相关语义规则并补回归测试。
- 修复缓存寄存器返回值读取。
- 再分阶段增强后端寄存器直用、CFG 清理、CSE 和尾递归优化。
