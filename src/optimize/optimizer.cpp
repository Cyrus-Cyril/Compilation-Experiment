...
std::vector<IRInstruction> hoistLoopInvariants(std::vector<IRInstruction> insts) {
    ...
    for (const auto& loop : loops) {
        ...
        for (size_t i = loop.bodyStart; i < loop.bodyEnd; ++i) {
            auto dest = destReg(insts[i]);
            if (dest && invariantRegs.count(*dest) &&
                isRemovableValueInst(insts[i]) &&
                insts[i].opcode != IROpcode::LOAD_LOCAL) {
                // LOAD_LOCAL 不能安全外提（可能被循环内的 STORE_LOCAL 修改）
        ...
        // 让所有从循环外跳入 header 的跳转改为跳转到 preheader
        // 查找所有跳转到 headerLabel 的指令（在循环外）
        for (size_t i = 0; i < loop.headerIdx; ++i) {
            auto& instRef = insts[i];
            if ((instRef.opcode == IROpcode::JMP || instRef.opcode == IROpcode::BEQ ||
                 instRef.opcode == IROpcode::BNE) && !instRef.operands.empty()) {
                IROperand& target = instRef.operands.back();
                if (target.kind == OperandKind::Label &&
                    labelId(target) == loop.headerLabel) {
                    // 非循环内跳转 → 重定向到 preheader
                    bool isFromLoopLatch = loop.bodyIndices.count(i);
                    if (!isFromLoopLatch) {
                        // 这个跳转在循环外，需要重定向
                        // 但我们在复制时已经处理了（headerIdx 之前的都复制了）
                        // 需要在 result 中修改最后一条相关指令
                        // 不对，这里不对。我们直接在最终复制时处理。
                    }
                }
            }
        }
        ...
        // 注意：result 中循环体已被重新排列，不能直接用原始索引判断
        // 改为：只修改在 preheader 之前的跳转（即循环外的入口跳转）
...
std::vector<IRInstruction> unrollLoops(std::vector<IRInstruction> insts) {
    ...
    for (const auto& loop : loops) {
        ...
        // 新 header：直接跳入展开体（如果无余数，epilogue 就是原始循环）
        ...
        // 跳转到原始循环处理剩余迭代（也可能直接退出）
        result.push_back({IROpcode::JMP, {IROperand::label(loop.headerLabel)}});

        // 新 exit label（循环后的指令使用）
